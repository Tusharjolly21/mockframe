...
/** Read intrinsic dimensions of a picked video via a detached element. */
function probeVideo(file: File): Promise<PromoMedia> {
  return new Promise((resolve, reject) => {
    ...
    v.muted = true;
    v.onloadedmetadata = () =>
      resolve({ id: `vid_${Math.random().toString(36).slice(2, 10)}`, url, width: v.videoWidth || 1080, height: v.videoHeight || 1920, kind: "video", name: file.name });
...
export default function PromoPanel({ onClose }: { onClose: () => void }) {
  ...
  const screenshots = useMemo(
    () => (media.length ? media.map((m) => ({ url: m.url, width: m.width, height: m.height, kind: m.kind })) : [PLACEHOLDER]),
    [media],
  );
  ...
  const inputProps = useMemo(
    () => buildPromoInputProps({ ...project, screenshotAssetIds: media.map((m) => m.id) }, { screenshots, watermark: !isPro }),
    [project, screenshots, media, isPro],
  );
  ...
  function selectTemplate(id: string) {
    setProject(createPromoProject(id, media.map((m) => m.id)));
    setStatus({ kind: "idle" });
  }
  async function onPickFiles(files: FileList) {
    const room = MAX_SCREENSHOTS - media.length;
    const picked = Array.from(files).slice(0, Math.max(0, room));
    const added = await Promise.all(
      picked.map(async (f): Promise<PromoMedia | null> => {
        try {
          if (f.type.startsWith("video/")) return await probeVideo(f);
          const a = await ingestFile(f);
          return { id: a.id, url: a.url, width: a.width, height: a.height, kind: "image", name: a.name };
        } catch {
          return null;
        }
      }),
    );
    const next = [...media, ...added.filter((m): m is PromoMedia => Boolean(m))].slice(0, MAX_SCREENSHOTS);
    setMedia(next);
    setProject((prev) => ({ ...prev, screenshotAssetIds: next.map((m) => m.id) }));
  }
  ...
  const canAddMore = media.length < MAX_SCREENSHOTS;
  const noScreens = media.length === 0;

  return (
    <div className="fixed inset-0 z-[85] flex bg-[#08080b] text-white">
      <input ref={fileRef} type="file" accept="image/*,video/mp4,video/webm,video/quicktime" multiple hidden onChange={(e) => { if (e.target.files?.length) void onPickFiles(e.target.files); e.target.value = ""; }} />
          ...
          {step === 1 && (
            <section>
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="text-[11px] text-zinc-600">{media.length}/{MAX_SCREENSHOTS}</span>
              </div>
              {noScreens ? (
                <button onClick={() => fileRef.current?.click()} className="fk-press flex h-40 w-full flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-white/20 bg-white/[0.03] text-[13px] font-semibold hover:bg-white/[0.06]">
                  <Upload size={18} /> Upload screenshots or a screen recording
                  <span className="text-[11px] font-normal text-zinc-500">Images and MP4/WebM recordings — the template cuts between them</span>
                </button>
              ) : (
                <div className="grid grid-cols-4 gap-2">
                  {media.map((m) => (
                    <div key={m.id} className="group relative aspect-[9/16] overflow-hidden rounded-lg border border-white/10 bg-black">
                      {m.kind === "video" ? (
                      ...
                      {m.kind === "video" && <span className="absolute bottom-1 left-1 rounded bg-black/70 px-1 py-0.5 text-[8px] font-bold uppercase text-white">Video</span>}
                      <button onClick={() => removeScreenshot(m.id)} title="Remove" className="absolute right-1 top-1 grid h-5 w-5 place-items-center rounded-full bg-black/70 opacity-0 transition-opacity group-hover:opacity-100"><X size={11} /></button>
                    </div>
                  ))}
              ...
              <p className="mt-3 text-[11px] leading-5 text-zinc-500">Tip: mix screenshots with a screen recording — the recording plays inside the phone.</p>
            </section>
          )}
          ...
          {step === 4 && (
            <section className="space-y-4">
              ...
              <div className="space-y-2 rounded-xl border border-white/10 bg-white/[0.03] p-4 text-[12.5px]">
                {[["Template", template.name], ["Screens", `${media.length}`], ["Device", DEVICES.find((d) => d[0] === project.deviceId)?.[1] ?? project.deviceId], ["Format", project.format], ["Length", `${Math.round(project.durationInFrames / PROMO_FPS)}s`]].map(([k, v]) => (
