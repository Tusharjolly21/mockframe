          ...
          {HOME_STORIES.map((story) => (
            <div key={story.id} className="flex flex-col">
              <div className="mx-auto w-[240px] max-w-full">
                <PhoneShell>
                  <LiveChatStory doc={story.doc} className="h-full w-full" />
                </PhoneShell>
              </div>
              <div className="mt-6 text-center">
                <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 px-2.5 py-0.5 text-[10.5px] font-semibold uppercase tracking-wide text-emerald-300">
                ...
                <p className="mx-auto mt-2 max-w-xs text-[13.5px] leading-relaxed text-zinc-400">{story.blurb}</p>
              </div>
            </div>
          ))}
        </div>
